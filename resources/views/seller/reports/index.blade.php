    ...
    tailwind.config = {
      theme: {
        extend: {
          ...
          boxShadow: {
            soft: '0 8px 30px rgba(2,6,23,.06)',    // สวยนวล
            card: '0 10px 35px rgba(2,6,23,.08)',
          },
          borderRadius: {
            xl2: '1rem',
          }
  ...
  <header class="sticky top-0 z-30 bg-white/80 backdrop-blur shadow-soft">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        {{-- <a href="{{ route('seller.index') }}" class="text-sm text-slate-600 hover:text-ink">Dashboard</a> --}}
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-12 gap-6">
    ...
    <aside class="col-span-12 md:col-span-3">
      <div class="bg-white rounded-2xl shadow-card p-4 md:p-5">
        <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Menu</h3>
        <nav class="space-y-1">
          <a href="{{ route('seller.reports.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-xl bg-slate-100 text-ink">
          ...
          <a href="{{ route('seller.products.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-100">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M20 6H4v12h16V6zM8 8h8v2H8V8zm0 4h8v2H8v-2z"/></svg>
            Product Management
          </a>
          <a href="{{ route('seller.orders.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-100">
    ...
    <main class="col-span-12 md:col-span-9 space-y-6">
      ...
      <section class="bg-white rounded-2xl shadow-soft p-6">
        ...
        <div class="flex items-start sm:items-center justify-between gap-3 mb-4">
            ...
            @php
            // รวมยอดทั้งหมดเพื่อไว้สรุปเล็ก ๆ (ไม่แสดงถ้าไม่มีข้อมูล)
            $totalAll = isset($dailySales) && $dailySales->isNotEmpty()
        ...
        @php
            $max = isset($dailySales) && $dailySales->isNotEmpty()
            ? max(1, (int) $dailySales->max('total'))
            : 1;
        @endphp
        ...
        @else
            ...
            <div class="flex items-end gap-1.5 h-28">
                ...
                <div
                    ...
                    style="
                    height: {{ $h }}%;
                    background: linear-gradient(180deg, rgba(59,130,246,0.85) 0%, rgba(147,197,253,0.95) 100%);
            ...
            <table class="min-w-full text-sm">
                ...
                @foreach($dailySales as $row)
                    ...
                    <td class="px-4 py-3 text-slate-700">
                        {{ \Illuminate\Support\Carbon::parse($row->d)->format('d/m/Y') }}
                    </td>
      ...
      {{-- Low stock list + Top products (2 columns) --}}
      <section class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {{-- Low stock --}}
        <div class="bg-white rounded-2xl shadow-card p-6">
          <div class="flex items-center justify-between mb-4">
            <h2 class="font-semibold">สินค้าใกล้หมด (Top 10)</h2>
            <span class="text-xs rounded-full bg-amber-50 text-amber-700 px-2 py-0.5">
              เหลือน้อยกว่าหรือเท่ากับ {{ $lowStockThreshold }}
            </span>
          </div>
              ...
              @foreach($lowStockList as $p)
                <li class="py-3 flex items-center justify-between hover:bg-slate-50/60 px-2 rounded-lg transition">
                  ...
                  <span class="text-xs rounded-full px-2 py-1
                    {{ $p->stock_quantity <= 2 ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700' }}">
